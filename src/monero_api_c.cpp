...
bool parse_network_type(int network_type, monero_network_type& net_type) {
  ...
  else if (network_type == MONERO_NETWORK_TESTNET) net_type = monero_network_type::TESTNET;
  else if (network_type == MONERO_NETWORK_STAGENET) net_type = monero_network_type::STAGENET;
  else return false;
...
bool parse_message_signature_type(int signature_type, monero_message_signature_type& sig_type) {
  if (signature_type == MONERO_MESSAGE_SIGN_WITH_SPEND_KEY) sig_type = monero_message_signature_type::SIGN_WITH_SPEND_KEY;
  else if (signature_type == MONERO_MESSAGE_SIGN_WITH_VIEW_KEY) sig_type = monero_message_signature_type::SIGN_WITH_VIEW_KEY;
  else return false;
...
std::string from_hex(const std::string& hex, std::string prefix = "0b") {
  ...
  if (hex.rfind(prefix, 0) == 0) start = prefix.size();
  if (((hex.size() - start) % 2) != 0) throw std::invalid_argument("Invalid hex string length");
...
void* monero_utils_get_integrated_address(int network_type, const char* standard_address, const char* payment_id) {
  DEBUG_START()
  if (standard_address == nullptr) return nullptr;

  monero_network_type net_type;
  if (!parse_network_type(network_type, net_type)) return nullptr;
